  ...
  <div class="audit-log-container">
    ...
    <div class="el-table-container">
      <el-table :data="paginatedAuditLogs" style="width: 100%" size="small">
        ...
        <el-table-column prop="approved_by" label="审核者">
          <template #default="scope">
            {{ scope.row.status === 'pending' ? '待审核' : (scope.row.approved_by || '未知') }}
          </template>
        </el-table-column>
        ...
        <el-table-column prop="approved_at" label="审核时间">
          <template #default="scope">
            {{ scope.row.status === 'pending' ? '待审核' : (scope.row.approved_at ? formatDateTime(scope.row.approved_at) : '未知') }}
          </template>
        </el-table-column>
      ...
      <div class="audit-detail-content">
        ...
        <el-card class="info-card mb-4">
          ...
          <div class="info-grid">
            ...
            <div class="info-item">
              <span class="info-label">审核者：</span>
              <span class="info-value">{{ currentAudit.approved_by || '未知' }}</span>
            </div>
            <div class="info-item">
            ...
            <div class="info-item">
              ...
              <span class="info-value">{{ currentAudit.approved_at ? formatDateTime(currentAudit.approved_at) : '未知' }}</span>
            </div>
          </div>
        </el-card>
...
const processedAuditLogs = computed(() => {
  if (isAdmin.value) {
    return auditLogs.value.filter(log => log.status !== 'withdrawn')
  } else {
    return auditLogs.value.filter(log => (log.status === 'approved' || log.status === 'rejected') && log.status !== 'withdrawn')
  }
})

const paginatedAuditLogs = computed(() => {
  const logs = filteredAuditLogs.value.length > 0 ? filteredAuditLogs.value : processedAuditLogs.value
